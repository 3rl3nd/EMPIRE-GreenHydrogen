...
def run_empire(name, tab_file_path, result_file_path, scenariogeneration, scenario_data_path,
    ...
    if hydrogen is True:
        ...
        if GREEN_H2:
            model.hydrogenGreenProducedElectro_ton = Var(model.HydrogenProdNode, model.Operationalhour, model.Period, model.Scenario, domain=NonNegativeReals)
            model.genOperationalGreen = Var(model.GeneratorsOfNode, model.Operationalhour, model.Period, model.Scenario, domain=NonNegativeReals)
            model.powerForGreenHydrogen = Var(model.HydrogenProdNode, model.Operationalhour, model.Period, model.Scenario, domain=NonNegativeReals)
        ...
        if GREEN_H2:
            model.elyzerGreenCapBuilt = Var(model.HydrogenProdNode, model.Period, domain=NonNegativeReals)
            model.elyzerGreenTotalCap = Var(model.HydrogenProdNode, model.Period, domain=NonNegativeReals)
            model.RESgenInstalledCap = Var(model.GeneratorsOfNode, model.Period, domain=NonNegativeReals)
        ...
        def prepElectrolyzerInvCost_rule(model):  #if GREEN_H2: Added p in electrolyzerplants atohugh all cost parameters are the same. Consider removing this 
            for i in model.Period:
    ...
    def Obj_rule(model):
        if hydrogen is True:
            returnSum = sum(model.discount_multiplier[i]*(sum(model.genInvCost[g,i]* model.genInvCap[n,g,i] for (n,g) in model.GeneratorsOfNode) + \
                                                          ...
                                                          sum(model.repurposedPipelineInvCost[n1,n2,i] * model.repurposedPipelineBuilt[n1,n2,i] for (n1,n2) in model.RepurposeDirectionalLinks) + \
            ...
            if GREEN_H2:         
                returnSum += sum(model.discount_multiplier[i] * sum(model.elyzerInvCost[i] * model.elyzerGreenCapBuilt[n,i] for n in model.HydrogenProdNode) for i in model.Period)
    ...
    if HEATMODULE:
        def FlowBalanceEL_rule(model, n, h, i, w):
            if hydrogen is False or n not in model.HydrogenProdNode:
                ...
                if GREEN_H2:
                    returnSum -= model.powerForGreenHydrogen[n,h,i,w]
                if n in model.NaturalGasNode:
    ...
    def naturalGas_terminal_capacity_rule(model, n, t, h, i, w):
        if "russiangas" in t.lower():
            return model.ng_terminalImport[n,t,h,i,w] <= model.ng_terminalCapacity[n,t,i]*RussianGasCapacityFactor
        else:
    ...
    def naturalGas_pipeline_capacity_rule(model,n,n2,h,i,w):
        if (n,n2) in model.NaturalGasDirectionalLink:
            if (n,n2) in model.RepurposeDirectionalLinks:
                return model.ng_transmission[n,n2,h,i,w] + sum(model.repurposedPipelineBuilt[n,n2,j] for j in range(1,i+1)) <= model.ng_pipelineCapacity[n,n2]
            else:
    ...
    if hydrogen is True:
        def repurpose_cap_rule(model,n1,n2,i):
            return model.repurposedPipelineBuilt[n1,n2,i] - model.ng_pipelineCapacity[n,n2] <= 0
        model.repurpose_cap = Constraint(model.RepurposeDirectionalLinks, model.Period)
        ...
        def lifetime_rule_pipeline(model,n1,n2,i):
            ...
            if (n1,n2) in model.RepurposeDirectionalLinks and (n2,n1) in model.RepurposeDirectionalLinks:
                return sum((model.hydrogenPipelineBuilt[n1,n2,j]+model.repurposedPipelineBuilt[n1,n2,j]*(ng_MWhPerTon/hydrogen_MWhPerTon)*repurposeEnergyFlowFactor+model.repurposedPipelineBuilt[n2,n1,j]*(ng_MWhPerTon/hydrogen_MWhPerTon)*repurposeEnergyFlowFactor) for j in model.Period if j>=startPeriod and j<=i) - model.totalHydrogenPipelineCapacity[n1,n2,i] == 0
            elif (n1,n2) in model.RepurposeDirectionalLinks:
                return sum((model.hydrogenPipelineBuilt[n1,n2,j]+model.repurposedPipelineBuilt[n1,n2,j]*(ng_MWhPerTon/hydrogen_MWhPerTon)*repurposeEnergyFlowFactor) for j in model.Period if j>=startPeriod and j<=i) - model.totalHydrogenPipelineCapacity[n1,n2,i] == 0
            elif (n2,n1) in model.RepurposeDirectionalLinks:
                return sum((model.hydrogenPipelineBuilt[n1,n2,j]+model.repurposedPipelineBuilt[n2,n1,j]*(ng_MWhPerTon/hydrogen_MWhPerTon)*repurposeEnergyFlowFactor) for j in model.Period if j>=startPeriod and j<=i) - model.totalHydrogenPipelineCapacity[n1,n2,i] == 0
            else:
        ...
        if GREEN_H2:
            def lifetime_rule_green_elyzer(model,n,i):
                startPeriod = 1
                if value(1+i-model.elyzerLifetime/model.LeapYearsInvestment)>startPeriod:
                    startPeriod=value(1+i-model.elyzerLifetime/model.LeapYearsInvestment)
                return sum(model.elyzerGreenCapBuilt[n,j] for j in model.Period if j>=startPeriod and j<=i) - model.elyzerGreenTotalCap[n,i] == 0
            model.installedCapDefinitionGreenElyzer = Constraint(model.HydrogenProdNode, model.Period, rule=lifetime_rule_green_elyzer)
        ...
        def pipeline_cap_rule(model,n1,n2,h,i,w):
            if (n1,n2) in model.HydrogenBidirectionPipelines:
                return model.hydrogenSentPipeline[(n1,n2),h,i,w] + model.hydrogenSentPipeline[(n2,n1),h,i,w] - model.totalHydrogenPipelineCapacity[(n1,n2),i] <= 0
            elif (n2,n1) in model.HydrogenBidirectionPipelines:
                return model.hydrogenSentPipeline[(n1,n2),h,i,w] + model.hydrogenSentPipeline[(n2,n1),h,i,w] - model.totalHydrogenPipelineCapacity[(n2,n1),i] <= 0
            else:
        ...
        def hydrogen_flow_balance_rule(model,n,h,i,w):
            ...
            if n in model.HydrogenProdNode:
                ...
                if GREEN_H2:
                    balance += model.hydrogenGreenProducedElectro_ton[n,h,i,w]
            if n in model.ReformerLocations:
        ...
        if not Reformer_H2:
            def reformer_off_rule(model,n,p,h,i,w):
                return model.ng_forHydrogen[n,p,h,i,w] == 0  
            model.hydrogen_production_reformer_off = Constraint(model.ReformerLocations,model.ReformerPlants, model.Operationalhour, model.Period, model.Scenario, rule=reformer_off_rule)

        def hydrogen_production_electrolyzer_capacity_rule(model,n,h,i,w):
            if ONLY_GREEN:
                return model.powerForHydrogen[n,h,i,w] == 0  
            else:
        ...
        if GREEN_H2:
            def lifetime_rule_RESgen(model, n, g, i):
                ...
                if value(1+i-(model.genLifetime[g]/model.LeapYearsInvestment))>startPeriod:
                    startPeriod=value(1+i-model.genLifetime[g]/model.LeapYearsInvestment)
                return sum(model.genInvCap[n,g,j] for j in model.Period if j>=startPeriod and j<=i)- model.RESgenInstalledCap[n,g,i] == 0   #
            model.installedRESCapDefinitionGen = Constraint(model.GeneratorsOfNode, model.Period, rule=lifetime_rule_RESgen)

            # Converting power to hydrogen mass 
            def hydrogen_green_production_rule(model,n,h,i,w):
                return model.hydrogenGreenProducedElectro_ton[n,h,i,w] == model.powerForGreenHydrogen[n,h,i,w] / model.elyzerPowerConsumptionPerTon[i]
            model.hydrogen_green_production = Constraint(model.HydrogenProdNode, model.Operationalhour, model.Period, model.Scenario, rule=hydrogen_green_production_rule)

            # Green capacity restriction
            def hydrogen_green_production_electrolyzer_capacity_rule(model,n,h,i,w):
                return model.powerForGreenHydrogen[n,h,i,w] <= model.elyzerGreenTotalCap[n,i]
            model.hydrogen_green_production_electrolyzer_capacity = Constraint(model.HydrogenProdNode, model.Operationalhour, model.Period, model.Scenario, rule=hydrogen_green_production_electrolyzer_capacity_rule)
    ...
    try:
        ...
        if HEATMODULE:
            if 'results_output_conv' in include_results:
        ...
        if 'results_output_offshoreConverter' in include_results:
        ...
        if HEATMODULE:
            if 'results_output_gen_el' in include_results:
            ...
            if 'results_output_gen_tr' in include_results:
        ...
        if 'results_CO2_sequestration_investments' in include_results:
        ...
        if 'results_industry_steel_investments' in include_results:
        ...
        if 'results_industry_steel_production' in include_results:
        ...
        if 'results_industry_cement_investments' in include_results:
        ...
        if 'results_industry_ammonia_investments' in include_results:
        ...
        if 'results_industry_cement_production' in include_results:
        ...
        if 'results_industry_ammonia_production' in include_results:
        ...
        if 'results_industry_oil_production' in include_results:
        ...
        if 'results_natural_gas_production' in include_results:
                            ...
                            for (s,h) in instance.HoursOfSeason:
                                if (n,t) in instance.NaturalGasTerminalsOfNode:
                                    ...
                                    my_string = [n,inv_per[int(i-1)],w,s,h,t,
                                                 value(instance.ng_terminalCapacity[n,t,i]*RussianGasCapacityFactor if ("russiangas" in t.lower()) else instance.ng_terminalCapacity[n,t,i]),
        ...
        if 'results_natural_gas_transmission' in include_results:
            ...
            writer = csv.writer(f)
            my_header = ["FromNode","ToNode","Period","Scenario","Season","Hour","Natural Gas transmitted [ton]","Tranmission Capacity [ton/hr]", "Total pipeline repurposed", "New Transmission Capacity [ton/hr]"]
            writer.writerow(my_header)
            for n in instance.NaturalGasNode:
                for n2 in instance.Node:
                            ...
                            for w in instance.Scenario:
                                for (s,h) in instance.HoursOfSeason:
                                    my_string = [n,n2,inv_per[int(i-1)],w,s,h,
                                                 ...
                                                 value(sum(instance.repurposedPipelineBuilt[n,n2,j] for j in range(1,i+1)) if (n,n2) in instance.RepurposeDirectionalLinks else instance.ng_pipelineCapacity[n,n2]),
                                                 value(instance.ng_pipelineCapacity[n,n2]-(sum(instance.repurposedPipelineBuilt[n,n2,j] for j in range(1,i+1)) if (n,n2) in instance.RepurposeDirectionalLinks else instance.ng_pipelineCapacity[n,n2]))
                                                 ]
                                    writer.writerow(my_string)
            f.close()
        ...
        if 'results_natural_gas_hydrogen' in include_results:
        ...
        if 'results_transport_electricity_operations' in include_results:
        ...
        if 'results_transport_hydrogen_operations' in include_results:
        ...
        if 'results_transport_naturalGas_operations' in include_results:
        ...
        if hydrogen is True:
            if 'results_hydrogen_production_investments' in include_results:
                if GREEN_H2:
                    print("{hour}:{minute}:{second}: Writing GREEN hydrogen investment results to results_hydrogen_production_investments.csv...".format(
                        hour=datetime.now().strftime("%H"), minute=datetime.now().strftime("%M"), second=datetime.now().strftime("%S")))
                    ...
                    my_header = ["Node","Period","New electrolyzer capacity [MW]", "Total electrolyzer capacity [MW]", "New electrolyzer capacity [ton/h]", "Total electrolyzer capacity [ton/h]",
                                "Expected annual power usage [GWh]","Expected annual electrolyzer hydrogen production [ton]", 'Expected electrolyzer capacity factor',
                                "New Green electrolyzer capacity [MW]", "Total Green electrolyzer capacity [MW]", "New Green electrolyzer capacity [ton/h]", "Total Green electrolyzer capacity [ton/h]",
                                "Expected annual Green power usage [GWh]","Expected annual Green electrolyzer hydrogen production [ton]", 'Expected Green electrolyzer capacity factor',
                                'New Reformer capacity [ton/h]', 'Total Reformer capacity [ton/h]',
                    ...
                    for n in instance.HydrogenProdNode:
                        for i in instance.Period:
                            ...
                            electrolyzerGreenCapacity = value(instance.elyzerGreenTotalCap[n,i] / instance.elyzerPowerConsumptionPerTon[i])
                            expectedElectrolyzerGreenProduction = value(sum(instance.sceProbab[w] * instance.seasScale[s] * instance.hydrogenGreenProducedElectro_ton[n,h,i,w] for (s,h) in instance.HoursOfSeason for w in instance.Scenario))
                            electrolyzerGreenCapFactor = (expectedElectrolyzerGreenProduction/(electrolyzerGreenCapacity*8760) if electrolyzerGreenCapacity > .001 else 0)
                            ...
                            writer.writerow([n,inv_per[int(i-1)],
                                            ...
                                            value(instance.elyzerGreenCapBuilt[n,i]),
                                            value(instance.elyzerGreenTotalCap[n,i]),
                                            value(instance.elyzerGreenCapBuilt[n,i] / instance.elyzerPowerConsumptionPerTon[i]),
                                            electrolyzerGreenCapacity,
                                            value(sum(instance.seasScale[s] * instance.sceProbab[w] * instance.powerForGreenHydrogen[n,h,i,w] for (s,h) in instance.HoursOfSeason for w in instance.Scenario) / 1000),
                                            expectedElectrolyzerGreenProduction,
                                            electrolyzerGreenCapFactor,
            ...
            if 'results_hydrogen_reformer_detailed_investments' in include_results:
            ...
            if 'results_hydrogen_production' in include_results:
                ...
                if GREEN_H2:
                    my_header = ["Node", "Period", "Scenario", "Season", "Hour", "Power for hydrogen [MWh]", "Electrolyzer production[ton]", "Power for Green hydrogen [MWh]", "Green Electrolyzer production[ton]", 'Reformer production [ton]','Total CO2 emission [ton CO2/ton H2]']
                    writer.writerow(my_header)
                    for n in instance.HydrogenProdNode:
                        for i in instance.Period:
                            for w in instance.Scenario:
                                for (s,h) in instance.HoursOfSeason:
                                    my_string = [n, inv_per[int(i-1)], w, s, h,
                                                ...
                                                value(instance.powerForGreenHydrogen[n,h,i,w]),
                                                value(instance.hydrogenGreenProducedElectro_ton[n,h,i,w])
                                                ]
                    ...
                    my_header = ["Node", "Period", "Scenario", "Season", "Hour", "Power for hydrogen [MWh]", "Electrolyzer production[ton]", 'Reformer production [ton]','Total CO2 emission [ton CO2/ton H2]']
                    writer.writerow(my_header)
                    for n in instance.HydrogenProdNode:
                        for i in instance.Period:
                            for w in instance.Scenario:
            ...
            if 'results_hydrogen_pipeline_investments' in include_results:
                ...
                writer = csv.writer(f)
                my_header = ["Between node", "And node", "Period", "Pipeline capacity built [ton/hr]", "Pipeline capacity repurposed [ton/hr]", "Sum pipeline capacity repurposed [ton/hr]","Pipeline total capacity [ton/hr]",
                ...
                for (n1,n2) in instance.HydrogenBidirectionPipelines:
                    for i in instance.Period:
                        if (n1,n2) in instance.RepurposeDirectionalLinks and (n2,n1) in instance.RepurposeDirectionalLinks:
                            my_string = [n1, n2, inv_per[int(i-1)],
                                        value(instance.hydrogenPipelineBuilt[n1,n2,i]),
                                        value((instance.repurposedPipelineBuilt[n1,n2,i] + instance.repurposedPipelineBuilt[n2,n1,i])*(ng_MWhPerTon/hydrogen_MWhPerTon)*repurposeEnergyFlowFactor),
                                        value((sum((instance.repurposedPipelineBuilt[n1,n2,j] + instance.repurposedPipelineBuilt[n2,n1,j]) for j in range(1,i+1)))*(ng_MWhPerTon/hydrogen_MWhPerTon)*repurposeEnergyFlowFactor),
                            ...
                            writer.writerow(my_string)
                        elif (n1,n2) in instance.RepurposeDirectionalLinks or (n2,n1) in instance.RepurposeDirectionalLinks:
                            my_string = [n1, n2, inv_per[int(i-1)],
                                        value(instance.hydrogenPipelineBuilt[n1,n2,i]),
                                        value((instance.repurposedPipelineBuilt[n1,n2,i])*(ng_MWhPerTon/hydrogen_MWhPerTon)*repurposeEnergyFlowFactor if (n1,n2) in instance.RepurposeDirectionalLinks else instance.repurposedPipelineBuilt[n2,n1,i]*(ng_MWhPerTon/hydrogen_MWhPerTon)*repurposeEnergyFlowFactor),
                                        value((sum(instance.repurposedPipelineBuilt[n1,n2,j] for j in range(1,i+1)))*(ng_MWhPerTon/hydrogen_MWhPerTon)*repurposeEnergyFlowFactor if (n1,n2) in instance.RepurposeDirectionalLinks else (sum(instance.repurposedPipelineBuilt[n2,n1,j] for j in range(1,i+1)))*(ng_MWhPerTon/hydrogen_MWhPerTon)*repurposeEnergyFlowFactor),
                                        value(instance.totalHydrogenPipelineCapacity[n1,n2,i]),
                                        value(instance.discount_multiplier[i] * (instance.hydrogenPipelineBuilt[n1,n2,i] * instance.hydrogenPipelineInvCost[n1,n2,i])),
                                        #TODO Veldig merkelig at dette deles på 1000 når hydrogenSentPipeline er i ton allerede. Er det heller meningen å få delt på antall timer i en sesong, for nå er denne her veldig rar.
                                        value(sum(instance.sceProbab[w]*instance.seasScale[s]*(instance.hydrogenSentPipeline[n1,n2,h,i,w] + instance.hydrogenSentPipeline[n2,n1,h,i,w])/1000 for (s,h) in instance.HoursOfSeason for w in instance.Scenario))]
            ...
            if 'results_CO2_pipeline_investments' in include_results:
